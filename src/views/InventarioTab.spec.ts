// inventory-tabs-redesign / Work Unit 1: extracted from InventarioView.spec.ts.
...
import InventarioTab from './InventarioTab.vue'
...
const montarVista = () =>
  mount({
    components: { InventarioTab },
    template: '<v-app><InventarioTab /></v-app>',
  }, {
...
describe('InventarioTab', () => {
  ...
  // -----------------------------------------------------------------------
  // inventory-tabs-redesign WU 1: StockAlertsList integration
  // -----------------------------------------------------------------------

  it('renders the stock alerts section', async () => {
    const wrapper = montarVista()
    await esperarCargaInicial()
  ...
  describe('category filter and sort controls', () => {
    ...
    it('filters items by "ingrediente" category', async () => {
      ...
      // Default: "Todos" shows both in the CRUD list
      const itemsTodos = wrapper.findAll('[data-testid^="mp-row-"]')
      const nombresTodos = itemsTodos.map((el) => el.text())
      expect(nombresTodos.some((n) => n.includes('Harina'))).toBe(true)
      expect(nombresTodos.some((n) => n.includes('Caja'))).toBe(true)

      // Click "Ingredientes" filter
      await wrapper.find('[data-testid="mp-filter-ingrediente"]').trigger('click')
      await flushPromises()

      // CRUD list should only contain Harina, not Caja
      const items = wrapper.findAll('[data-testid^="mp-row-"]')
      const nombres = items.map((el) => el.text())
      expect(nombres.some((n) => n.includes('Harina'))).toBe(true)
      expect(nombres.some((n) => n.includes('Caja'))).toBe(false)
    })
    ...
    it('filters items by "empaque" category', async () => {
      ...
      // CRUD list should only contain Caja, not Harina
      const items = wrapper.findAll('[data-testid^="mp-row-"]')
      const nombres = items.map((el) => el.text())
      expect(nombres.some((n) => n.includes('Harina'))).toBe(false)
      expect(nombres.some((n) => n.includes('Caja'))).toBe(true)
    })
