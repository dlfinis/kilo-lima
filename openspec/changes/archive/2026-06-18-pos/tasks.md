...
- [x] 1.1 Write `src/utils/cierre.spec.ts` (RED) — `calcularCierre` edge cases (empty, mixed pago, diferencia ±/0/null, float-drift) + `formatearDiferencia`
- [x] 1.2 Implement `src/utils/cierre.ts` (GREEN) — `calcularCierre(input)` + `formatearDiferencia(monto)`
- [x] 1.3 Create `src/types/pos.types.ts` — Domain types: Producto, Venta, VentaItem, GastoImprevisto, CierreCaja, enums, *Input, cart/cierre shapes
- [x] 1.4 Extend `src/types/database.types.ts` — 5 tables (Row, Insert, Update, Relationships)
- [x] 1.5 Modify `src/types/index.ts` — Re-export pos types
- [x] 1.6 Create `supabase/migrations/20260619000000_pos_inicial.sql` — 5 tables (idempotent), FKs, CHECKs, indexes, RLS, updated_at trigger on productos
- [x] 1.7 Extend `supabase/dev_bypass_rls.sql` — Grant anon to 5 new tables
...
- [x] 4.1 Write `src/services/gastosImprevistos.service.spec.ts` (RED) → implement service (GREEN) — CRUD per-evento
- [x] 4.2 Write `src/stores/gastosImprevistos.store.spec.ts` (RED) → implement store (GREEN) — EVENTO_CERRADO guard via `estadoEsEditable`
- [x] 4.3 Implement `src/composables/useGastosImprevistos.ts` — Thin store wrapper
- [x] 4.4 Write `src/services/cierres.service.spec.ts` (RED) → implement service (GREEN) — `obtenerPorEvento`, `crear`, no `actualizar`
- [x] 4.5 Write `src/stores/cierres.store.spec.ts` (RED) → implement store (GREEN) — `registrarCierre` (insert + cambiarEstado), UNIQUE violation
- [x] 4.6 Write `src/composables/useCierreCaja.spec.ts` (RED) → implement `useCierreCaja.ts` (GREEN) — Wraps cierres.store + `calcularCierre` + `transicionEstadoValida`
- [x] 4.7 Write 3 component specs (RED) — GastoImprevistoForm, GastoImprevistoListItem, CierreResumenCard
- [x] 4.8 Implement 3 components (GREEN) — Form (monto>0, motivo≤500), ListItem row, ResumenCard (4 sections + yellow alert on diferencia≠0)
- [x] 4.9 Write `src/views/PosCierreView.spec.ts` (RED) → implement view (GREEN) — Cierre page with cash-count inputs, confirm dialog, redirect, retroactive cierre support

## PR 5 — Router + Cross-slice + Docs + Verify

- [x] 5.1 Modify `src/router/routes.ts` — Append lazy routes: `/pos` → PosView, `/pos/cierre/:eventoId` → PosCierreView (props:true)
- [x] 5.2 Modify `src/router/routes.spec.ts` — 2 new assertions for route resolution
- [x] 5.3 Modify `src/views/RecetaDetalleView.vue` — Add "Vender esta receta" button + precio_venta dialog when no producto exists; add 2 tests to `.spec.ts`
- [x] 5.4 Create `docs/pos-setup.md` — One-time setup: migration + bypass + cart-refresh caveat
- [x] 5.5 Final verify — `pnpm test` (≥80 passing), `pnpm typecheck` (0), `pnpm build` (passes)
